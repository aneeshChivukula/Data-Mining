...
def save_samples(samples,savefile):
#	samples[0] = 10.12
...
#	print(samples[0][samples[0]>1])
	...
	a = a.reshape((32,32,3)).astype(np.uint8)
#	print('a.shape',a.shape)
#	print('a[a!=0]',a[a!=0])
#	b = samples[0]*255
#	print('b.shape',b.shape)
#	print('b[b!=0]',b[b!=0])
#	print('a[a!=0]',type(a[a!=0][0]))
#	Image.fromarray(a).convert('RGB').save('/home/achivuku/Desktop/Conditional-DCGAN-master/samples/a.png')

	print('Why Here')
	sys.exit()
    
	samples = [sample.astype(np.int32) for sample in samples]
  ...
  if option == 0:
    ...
    for idx in xrange(100):
      ...
      if config.dataset == "mnist":
        y = np.random.choice(labels, config.batch_size)
        ...
        samples = sess.run(dcgan.sampler, feed_dict={dcgan.z: z_sample})
      save_images(samples, y, [image_frame_dim, image_frame_dim], './samples/test_arange_%s.png' % (idx), './samples/images/test_arange_%s-images.pkl' % (idx), './samples/labels/test_arange_%s-classes.pkl' % (idx))
  elif option == 2:
    values = np.arange(0, 1, 1./config.batch_size)
