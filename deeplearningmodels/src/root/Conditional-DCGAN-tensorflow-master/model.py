...
#     y = np.concatenate((trY, teY), axis=0).astype(np.int)
    ...
    selY = np.logical_or(trY==labels[0], trY==labels[1])
    ...
    for x in Xg:
        im = Image.fromarray(x[:,:,0]).resize((32,32), Image.NEAREST)
        xp = np.array(im.convert('RGB'), np.int32)
        X.append(xp)
...
#     if(label==labels[0] or label==labels[1]):
